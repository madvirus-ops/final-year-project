...
os.makedirs("histog", exist_ok=True)
...
for column in data.columns:
    ...
    for i, year in enumerate(unique_years):
        ...
        plt.hist(year_data['YEAR'], bins=len(year_data), color=color_map(i), label=str(year), alpha=0.7, edgecolor='black')
    ...
    plt.savefig(f'histog/{sanitized_column_name}_data_points_vs_year.png')

    plt.close()  # Close the current figure

# Optionally, display a message indicating the completion
print("Histograms saved to the 'histog' directory.")
